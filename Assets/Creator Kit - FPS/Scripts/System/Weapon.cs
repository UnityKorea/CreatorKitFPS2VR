...
public class Weapon : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        m_InteractableBase.selectExited.AddListener(PutDown);
    ...
    void PickedUp(SelectEnterEventArgs args)
    {
        ...
        //Debug.Log(" interactableObject ------------------- " + args.interactableObject.transform.gameObject.name);
        //Debug.Log(" interactorObject ------------------- " + args.interactorObject.transform.gameObject.name);

        var visual = args.interactorObject.transform.gameObject.GetComponent<XRInteractorLineVisual>();
        if (visual)
        {
    ...
    void PutDown(SelectExitEventArgs args)
    {
        triggerDown = false;
    ...
    void RaycastShot()
    {
        ...
        Ray r = new Ray(EndPoint.position, gameObject.transform.forward);
        ...
        if (Physics.Raycast(r, out hit, 1000.0f, ~(1 << 9), QueryTriggerInteraction.Ignore))
        {
            ...
            {
                var hitpoint = hit.collider.gameObject.GetComponent<TargetHitPoint>();
                if(hitpoint)
                    hitpoint.Got(damage);
                else
                {
